...
class Person:
    ...
    def need(self):
        ...
        for position in heirarchy_dictionary:
            if heirarchy_dictionary[position] == 2: # make it 3 later
    ...
    def neighbors_Iam_conflicting_with(self):
        ...
        if self.conflicts:
            ...
            #tag = " I am having a conflict with: {} in {}".format(neighbors_names, value)
            ...
            """
            if value == "need":
                self.personal_log.append("Need conflicts with{}".format(neighbors_in_need))
                return neighbors_in_need

            if value == "desire":
                print("value is desire but I am not returning neigbors")
                return neighbors_in_desire
            if value == "both":
                return neighbors_as_objects
            else:
                print("I don't know what's wrong in neighbors conflicts types")
            """
            #return neighbors
        else:
    ...
    def vectors_from_neighbors_OLD(self):
        ...
        all_neighbors = self.neighbors_Iam_conflicting_with(value)#??????
        print("_________________________",all_neighbors)
        ...
        if all_neighbors:
            ...
            if all_vectors:
                ...
                if value == "need":
                    self.personal_log.append("These are dominant vectors away from neighbors NEED {}".format(dominant_vectors))
                    self.vectors_away_from_neighbors_NEED = dominant_vectors
                    print("I did find vectors for neighbors NEED")


                elif value == "desire":
                    self.personal_log.append("These are dominant vectors away from neighbors DESIRE {}".format(dominant_vectors))
                    self.vectors_away_from_neighbors_DESIRE = dominant_vectors

                elif value == "both":
                    self.personal_log.append("These are dominant vectors away from neighbors BOTH {}".format(dominant_vectors))
                    self.vectors_away_from_neighbors_BOTH = dominant_vectors
                ...
                return False
                #print("I couldn't find vectors from neighbors!")
